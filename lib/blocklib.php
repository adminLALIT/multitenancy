...
class block_manager
{
    ...
    public function get_addable_blocks()
    {
        ...
        foreach ($allblocks as $block) {
            ...
            if (
                $block->visible && !in_array($block->name, $undeletableblocks) &&
    ...
    {
        return new moodle_url($this->page->url, array(
            'bui_moveid' => $this->movingblock,
    ...
    public function load_blocks($includeinvisible = null)
    {
        ...
        $rolecondition = '';
        $getrole = optional_param('role', null, PARAM_RAW);
        ...
        if (!empty($requiredbytheme)) {
            ...
            list($testnotsql, $requiredbythemenotparams) = $DB->get_in_or_equal(
                $requiredbytheme,
                SQL_PARAMS_NAMED,
                'notrequiredbytheme',
                false
        ...
        $rolejoin = "JOIN {role_based_blocks} rbb ON bi.id = rbb.blockinstance";
        $rolecondition = 'rbb.role = :filterrole AND rbb.companyid = :filtercompany';
       
        $params['filtercompany'] = $companyid;
        if (!is_null($getrole) && $getrole != '') {
            $params['filterrole'] = $getrole;
        } 
        else {
            if ($getrole = $DB->get_field_sql("SELECT r.shortname FROM {role} r JOIN {role_assignments} ra ON ra.roleid = r.id WHERE ra.userid = :userid AND ra.contextid = 1", ['userid' => $USER->id])) {
                $params['filterrole'] = $getrole;
            } elseif(is_siteadmin()) {
                $params['filterrole'] = 'admin';
            }
            else {
                $params['filterrole'] = 'student';
            }
        }
    ...
    {
        global $DB, $SESSION;
        $forrole  = optional_param('role', null, PARAM_RAW);
        ...
        if (!is_null($forrole) && !empty($SESSION->currenteditingcompany)) {
            $saverecord = new stdClass;
            $saverecord->timecreated = time();
            $saverecord->companyid =  $SESSION->currenteditingcompany;
            $saverecord->role = $forrole;
            $saverecord->blockinstance = $blockinstance->id;
            $DB->insert_record('role_based_blocks', $saverecord);
        }
    ...
    public function reposition_block($blockinstanceid, $newregion, $newweight)
    {
        ...
        $bi = $inst->instance;
        if (
            $bi->weight == $bi->defaultweight && $bi->region == $bi->defaultregion &&
    ...
    protected function create_block_contents($instances, $output, $region)
    {
        ...
        foreach ($instances as $instance) {
            ...
            if (
                $this->movingblock && $lastweight != $instance->instance->weight &&
            ...
            if ($content->blockinstanceid == $this->movingblock) {
                ...
                $content->annotation .= get_string(
                    'movingthisblockcancel',
                    'block',
    ...
    public function edit_controls($block)
    {
        ...
        if (get_assignable_roles($block->context, ROLENAME_SHORT)) {
            $rolesurl = new moodle_url('/admin/roles/assign.php', array(
        ...
        if (has_any_capability(array('moodle/role:safeoverride', 'moodle/role:override', 'moodle/role:assign'), $block->context)) {
            ...
            $controls[] = new action_menu_link_secondary(
                ...
                $str,
                array('class' => 'editing_checkroles')
            );
        }
    ...
    public function process_url_delete()
    {
        ...
        if (!$confirmdelete) {
            ...
            if ($DB->record_exists('role_based_blocks', ['blockinstance' => $block->instance->id])) {
                $DB->delete_records('role_based_blocks', ['blockinstance' => $block->instance->id]);
            }
    ...
    protected function get_secondarynav(block_base $block): \core\navigation\views\secondary
    {
        ...
        if (class_exists("block_{$block->name()}\\navigation\\views\\secondary")) {
            ...
            debugging(
                "The class block_{$block->name()}\\local\\views\\secondary uses a deprecated " .
